...
class Visualization(Prediction):
    ...
    def prediction_graph(self, equity = None):
        ...
        self.fig_action.add_trace(go.Bar(x = self.df_visualization.index, y = self.df_visualization['Bullish Volume'], name = "Bullish Volume", opacity = 0.25,
        marker = dict(color = '#008000')), secondary_y = True)
        self.fig_action.add_trace(go.Bar(x = self.df_visualization.index, y = self.df_visualization['Bearish Volume'], name = "Bearish Volume" , opacity = 0.25,
